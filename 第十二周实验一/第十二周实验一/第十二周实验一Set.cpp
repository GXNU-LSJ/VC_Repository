...
// 第十二周实验一Set.cpp : C第十二周实验一Set 类的实现
//
...
IMPLEMENT_DYNAMIC(C第十二周实验一Set, CRecordset)

C第十二周实验一Set::C第十二周实验一Set(CDatabase* pdb)
	: CRecordset(pdb)
{
	m_ID = 0;
	column1 = L"";
...
// 储为其他格式或使用其他的用户身份验证。
CString C第十二周实验一Set::GetDefaultConnect()
{
...
CString C第十二周实验一Set::GetDefaultSQL()
{
	return _T("[学生信息管理系统]");
}

void C第十二周实验一Set::DoFieldExchange(CFieldExchange* pFX)
{
...
#ifdef _DEBUG
void C第十二周实验一Set::AssertValid() const
{
	CRecordset::AssertValid();
}

void C第十二周实验一Set::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
